...
int mythread_create (void (*fun_addr)(),int priority)
{
	...
	if(tcb->priority == HIGH_PRIORITY)
	{
		...
		if(actualProcess->priority == HIGH_PRIORITY)
		{
			...
			/*EXPULSION -- LLEGA UNO DE ALTA PRIORIDAD .. Y HAY UNO DE BAJA -- SE CUELA EL NUEVO DE ALTA SIEMPRE*/
			//printf("swap\n");
...
void timer_interrupt(int sig)
{
	...
	/*START - CONTROL HAMBRUNA*/
	struct my_struct* auxElement = lowReadyQueue->head;
	TCB *auxProcess;
	while(NULL!=auxElement)
	{
		auxProcess=auxElement->data;
		auxProcess->hungry--;
		if(auxProcess->hungry==0)
		{
			//printf("TENGO HAMBRE %d - id %d\n",auxProcess->hungry,auxProcess->tid);
			/*CAMBIAMOS A COLA ALTA PRIORIDAD*/
			printf(" *** THREAD %d MOVED TO HIGH PRIORITY QUEUE\n",auxProcess->tid);
			auxProcess->hungry=STARVATION;
			auxElement=auxElement->next;
			auxProcess=dequeue(lowReadyQueue);
			enqueue(highReadyQueue,auxProcess);
		}else{
			auxElement=auxElement->next;
		}
	}
	/*END - CONTROL HAMBRUNA*/
